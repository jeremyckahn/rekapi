...
import { Rekapi, DOMRenderer, Actor } from '../src/main';
import {
  interpolate,
} from 'shifty';
...
describe('DOM renderer', () => {
  ...
  describe('constructor', () => {
    it('is a function', () => {
      assert.strictEqual(typeof DOMRenderer, 'function');
    });
  });
  ...
  describe('setting inline styles', () => {
    it('interpolates and sets transform styles', () => {
      ...
      assert.strictEqual(
        (actor.context as HTMLElement).style.transform,
    ...
    it('supports independent transform properties', () => {
      ...
      const { style } = actor.context as HTMLElement;
      ...
      assert.strictEqual(style.height, '25px');
    });
    ...
    it('supports translate3d', () => {
      ...
      const transformChunks = (
        actor.get() as { transform: string }
      ).transform.match(/(\d|\.)+/g)!;

      assert.strictEqual(
        transformChunks[1],
        interpolate({ x: 0 }, { x: 1 }, 0.5, 'linear').x.toString()
      );
      assert.strictEqual(
        transformChunks[2],
        interpolate({ x: 0 }, { x: 1 }, 0.5, 'easeInQuad').x.toString()
      );
      assert.strictEqual(
        transformChunks[3],
        interpolate({ x: 0 }, { x: 1 }, 0.5, 'easeOutQuad').x.toString()
      );
    });
    ...
    it('supports "3deg" value', () => {
      ...
      assert.strictEqual(
        (actor.get() as { transform: string }).transform.match(
          /(\d|\.)+/g
        )![0],
        interpolate({ x: 3 }, { x: 6 }, 0.5, 'linear').x.toString()
      );
    });

    it('supports decoupled unit-less transform values', () => {
      actor.keyframe(0, { scale: 0 }).keyframe(100, { scale: 1 });

      rekapi.update(0);

      assert.strictEqual(
        (actor.context as HTMLElement)
          .getAttribute('style')!
          .match(/transform:.*?;/)![0],
        'transform: scale(0);'
      );
  ...
  describe('#setTransformOrder', () => {
    it('throws an exception if given an unknown/unsupported transform function', () => {
      assert.throws(() =>
        rekapi
          .getRendererInstance(DOMRenderer)
          .setActorTransformOrder(actor, ['foo', 'bar', 'rotate'])
      );
    });
    ...
    it('sets a transform property order', () => {
      ...
      rekapi
        .getRendererInstance(DOMRenderer)
        .setActorTransformOrder(actor, order);
      assert.deepStrictEqual(actor._transformOrder, order);
    });
    ...
    it('ignores duplicate values passed to setTransformOrder', () => {
      ...
      rekapi
        .getRendererInstance(DOMRenderer)
        .setActorTransformOrder(actor, order);
      assert.deepStrictEqual(actor._transformOrder, ['rotate', 'translateX']);
    });
    ...
    it('sets inline styles in specified order', () => {
      ...
      rekapi
        .getRendererInstance(DOMRenderer)
        .setActorTransformOrder(actor, ['rotate', 'translateY', 'translateX']);
