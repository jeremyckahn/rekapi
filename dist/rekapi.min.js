...
m(this,"onBeforeDraw");var c=this._drawOrder.length,f;this._drawOrderSorter?(f=e.sortBy(this._actors,this._drawOrderSorter),f=e.pluck(f,"id")):f=this._drawOrder;var a,d,j;for(j=0;j<c;j++)a=this._actors[f[j]],d=a.context(),a.render(d,a.get());this._lastRenderedMillisecond=b;m(this,"onFrameRender");return this};a.prototype.redraw=function(){this.render(this._lastRenderedMillisecond);return this};a.prototype.calculateActorPositions=function(b){var c=this._drawOrder.length,f;for(f=0;f<c;f++)this._actors[this._drawOrder[f]].calculatePosition(b);
